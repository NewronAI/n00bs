...
if(!existsSync(`/data2/data_nginx/single_audio/${vendor}/notFoundImages/notFoundfiles/${batch}`)) {
    await exec(`mkdir /data2/data_nginx/single_audio/${vendor}/notFoundImages/notFoundfiles/${batch}`)
    console.log(`Created Folder /data2/data_nginx/single_audio/${vendor}/notFoundImages/notFoundfiles/${batch}`)
}

if(!existsSync(`/data2/data_nginx/single_audio/${vendor}/audioLinks/${batch}`)) {
    await exec(`mkdir /data2/data_nginx/single_audio/${vendor}/audioLinks/${batch}`)
    console.log(`Created the directory /data2/data_nginx/single_audio/${vendor}/audioLinks/${batch}`)
}
...
let data = {
    ...
    "baseAudioLocation": `/data/Database/manual_qc/SingleAudio_QC2/${vendor}/${batch}/Audios`,
    "imagesDirPath": "/data2/data_nginx/single_audio/Images/Images_Mar23",
    "csvFilePath" : csvFilename,
    "imageNotFoundDataCsvPath" : `/data2/data_nginx/single_audio/${vendor}/notFoundImages/notFoundfiles/${batch}`,
    "logsPath" : `/data2/data_nginx/single_audio/${vendor}/logs/${batch}`,
    "resultCSV" : `/data2/data_nginx/single_audio/${vendor}/audioLinks/${batch}`
}
...
fs.writeFile(`/data2/data_nginx/single_audio/${vendor}/configFiles/audioLinks/${filename}`, JSON.stringify(data), err => {
  if (err) throw err;
