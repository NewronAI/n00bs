...
assignedFilesAPI.get(async (req, res) => {
    ...
    const logger = getLogger(`/api/v1/${workflowUUID}/file/assigned`);
    ...
    const assignedFiles = await db.$queryRaw`WITH
            wf_id AS (
    ...
    logger.debug(`assignedFiles : ${assignedFiles && typeof assignedFiles === 'object' && JSON.stringify(assignedFiles)}`);
