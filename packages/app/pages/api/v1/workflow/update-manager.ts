...
const updateManagerHandler = async (req: NextApiRequest, res: NextApiResponse) => {

    const { method } = req;

    const logger = getLogger("api/manager/v1/workflow/update-manager.ts");
    ...
    try {
        ...
        const workflowManagerLink = await db.workflow_manager_link.findFirst({
            where: {
        ...
        assertUp(workflowManagerLink, {
            message: "Manager not found in workflow",
            status: 404
        })

        await db.workflow_manager_link.update({
            data: {
                role
            },
            where: {
                id: workflowManagerLink.id
            }
        });
...
export default updateManagerHandler;